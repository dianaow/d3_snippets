var moving_bubbles = function () {
  ...
  var centroids, progress, circles
  ...
  function init() {
    d3.csv("./data/scores_trunc.csv", function(data) {
      ...
      d3.select('.changeFocitoCrime').on("click", function () { init() });
      ...
      d3.select('.run_change_foci').on("click", function () { simulate_group(nodes, crimeFoci) });
  ...
  function simulate_group(nodes, foci) {

    if(NUM_FLICKS){
      setTimeout(function() { changeNodeGroup(NUM_FLICKS, foci) }, 1000)
    }
    ...
    function changeNodeGroup(NUM_FLICKS, foci) {
